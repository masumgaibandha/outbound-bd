...
import { agencyAttributionSchema } from "@/lib/agency-inquiry-schema";
...
import { Inquiry, type InquiryDocument, type InquirySource } from "@/lib/models/inquiry";
import { connectToDatabase } from "@/lib/mongoose";

/**
 * The one POST handler behind every paid-traffic landing-page form
 * (`/api/agencies-lead`, `/api/cold-email-lead`). Each route is a thin
 * wrapper that passes its own source, schema and auto-reply wording; the
 * protections, persistence order and best-effort side effects are
 * identical for every landing page and live only here.
 */

/** What every landing form's schema produces. The size question is `activeClients` on /agencies and `teamSize` on /cold-email. */
...
export interface LandingLeadRouteConfig {
  route: string;
  source: Exclude<InquirySource, "contact">;
  schema: z.ZodType<LandingLeadInput>;
  /** Completes the auto-reply's "Thanks for reaching out about cold email for ___." line. */
  autoReplyTopic: string;
}
...
// to the route's own source: a landing-page lead has no "company" field to
// key on the way /api/inquiries's duplicate check does.
const DUPLICATE_WINDOW_MS = 5 * 60 * 1000;

export async function handleLandingLeadPost(
  request: Request,
  { route, source, schema, autoReplyTopic }: LandingLeadRouteConfig,
  ...
  if (!autoReplyResult.ok) {
    ...
    // best-effort failure log in this handler — never the visitor's name
    // or email.
    console.error(
      JSON.stringify({
